...
class Enemy(CharacterBase):
    ...
    def _is_okay_to_go(self, axis: "float"):
        ...
        base_distance = self._floor_check_distance * int(abs(axis))
        rechecks = 1
        axis = util.clamp(axis, -1, 1)
        ...
        for divstep in range(1, rechecks + 1):
            ...
            if self._floor_allow_stair:
                is_okay = not collides(to_wall) and (
                    collides(to_floor_1) or collides(to_floor_2)
                )
            else:
...
class Boss(Enemy):
    ...
    def next_pattern(self):
        next = self.current_pattern.next
        if not next:
            next = (self._pattern_idx + 1) % len(self.patterns)
        ...
        if old_pattern_index != pattern_idx:
            if DEBUG_MODE:
                print(
                    ...
                    f"Switch Boss Pattern {self._pattern_idx} -> {pattern_idx}",
                )
...
def pattern_jump(self: "Boss", command: "MoveCommand"):
    command.jump = True

def pattern_do_nothing(self: "Boss", command: "MoveCommand"):
    pass
...
class WindEnemy(Boss):
    def pattern_shoot_wind(self: "WindEnemy", command: "MoveCommand"):
        self._weapon = self._weapon_blower
        pattern_shoot(self, command)

    def pattern_shoot_tornado(self: "WindEnemy", command: "MoveCommand"):
...
class SpeedEnemy(Boss):
    ...
    def pattern_move_left_quick(self: "Boss", command: "MoveCommand"):
